...
const AdditionalGuest = ({ guest }) => {
  ...
  const handleInputChange = (name, value) => {
    setReservationData((prev) => {
      const updatedGuests = prev.additional_guests.map((g) =>
        g.id === guest.id ? { ...g, [name]: value } : g,
      );
  ...
  const removeAdditionalGuest = (id) => {
    setReservationData((prev) => {
      const updatedAdditionalGuests = prev.additional_guests.filter(
        (guest) => guest.id !== id,
      );
    ...
    <div className="mx-5 mb-6 flex flex-row">
      ...
      <div className="relative flex flex-row items-center justify-center">
        <button
          ...
          className="rounded-lg p-1.5 text-red-500 dark:hover:bg-gray-600 dark:hover:text-red-600"
        >
          <svg
            className="size-4"
            fill="none"
