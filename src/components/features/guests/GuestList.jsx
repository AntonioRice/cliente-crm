...
const GuestsList = ({ guests, columns }) => {
  ...
  return (
    <tbody>
      {guests.map((guest) => (
        <tr key={guest.guest_id} onClick={() => handleRowClick(guest)} className="cursor-pointer border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-[#282828] dark:hover:bg-gray-600 dark:hover:text-green-400">
          {columns.map((col) => (
